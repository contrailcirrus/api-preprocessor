...
resource "google_bigquery_dataset" "api_preprocessor_prod" {
  dataset_id = "api_preprocessor_prod"
  friendly_name = "[PROD] api preprocessor"
  description = "data assets generated by the api preprocessor"
  location = "US"
  delete_contents_on_destroy = false
}

resource "google_bigquery_table" "cocip_regions_prod" {
  dataset_id = google_bigquery_dataset.api_preprocessor_prod.dataset_id
  table_id   = "cocip_regions_prod"
  friendly_name = "[DEV] cocip regions"
  description = "region polygons generated from CoCip"
  deletion_protection = true
  time_partitioning {
